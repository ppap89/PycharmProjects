...
def save_img2():  # 提取视频中图片 按照每秒提取   间隔是视频帧率
    video_path = r'G:/observer_compute_intelligence/data/RawData/20220225/videos0/'  # 视频所在的路径
    f_save_path = 'G:/observer_compute_intelligence/data/RawData/20220225/videos0_images/'  # 保存图片的上级目录
        ...
        while rval:  # 循环读取视频帧
            ...
            if rval:
                ...
                if (c % round(fps) == 0):  # 每隔fps帧进行存储操作   ,可自行指定间隔
                    cv2.imwrite(pic_path + str(round(c/fps)) + 'jpg', frame) #存储为图像的命名 video_数字（第几个文件）.png
                    print(str(round(c/fps)) + '.jpg')
