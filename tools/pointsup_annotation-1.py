...
usage:
    ...
    2. for visualize:
    python pointsup_annotation.py --json_file path/to/data.json --data_root path/to/datadir --visualize [--scale 0.8]
...
class PointSupAnnotate(object):
    ...
    """
    def __init__(self, json_file, data_root, scale=1.0, point_num=10, visualize=False):
    ...
    def _save(self):
        ...
        print(f"new JSON file saved in {self.new_json}.")
    ...
    def show(self):
        ...
        """
        img_count = 0
        while img_count < len(self.coco.dataset["images"]):
            img_obj = self.coco.dataset["images"][img_count]
            imid = img_obj["id"]
            ...
            while ann_count < len(anns):
                ...
                if c == 27 or c == ord('q'):
                    ...
                    F1 = open('C:\\Users\\Administrator\\Desktop\\1\\re2.txt','a')
                    F1.write(str(imid-1))
                    F1.write('\n')
...
if __name__ == '__main__':
    ...
    if method == 0:
        ...
        json_file = "W:\\data\\DevData\\22\\annotations\\instances_default1.json"
        data_root = "W:\\data\\DevData\\22\\images"
        img_scale = 0.8
        visualize = False       # False for annotation

        psa = PointSupAnnotate(json_file, data_root, scale=img_scale, visualize=visualize)
        psa()
